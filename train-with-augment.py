...
def hard_train_and_test(params, transform):
    model = CNN_with_Unet(
        in_channels=5,
        out_channels=1,
...
def tune_parameters(n_trials, transform_name=None, transform=None):
    ...
    study = optuna.create_study(
        ...
        direction="maximize",
        sampler=optuna.samplers.TPESampler(),  # should do seed for reproducability but there is a bug using seed with running multiple jobs so can't
    ...
    study.optimize(func, n_trials=n_trials, n_jobs=2)
